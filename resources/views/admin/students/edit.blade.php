    ...
    <form action="{{ route('students.update', $student->id) }}" method="POST" autocomplete="off">
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" id="full_name" placeholder="Enter full name" value="{{ old('name', $student->name) }}">
                @error('name')
                <span class="text-danger small">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                <div class="custom-control custom-switch">
                    <input type="checkbox" name="active" class="custom-control-input" id="active" value="1" {{ old('active', $student->active) ? 'checked' : '' }}>
